...
`get_context_size` uses the GPT-6 family default for any name beginning with
`gpt-6`, including variants and snapshots. GPT-6 tokenizer lookup and token-budget
helpers are unsupported because no encoding mapping is available.
