...
pub fn num_tokens_from_messages(
    ...
    // Token overhead constants from the OpenAI cookbook:
    // https://github.com/openai/openai-cookbook/blob/main/examples/How_to_count_tokens_with_tiktoken.ipynb
    //
    // tokens_per_message: overhead tokens added per message for framing
    //   gpt-3.5-turbo-0301: 4 tokens (<im_start>{role/name}\n{content}<im_end>\n)
    //   all other models:   3 tokens
    //
    // tokens_per_name: extra tokens when a message includes a `name` field
    //   gpt-3.5-turbo-0301: -1 (role is omitted when name is present)
    //   all other models:    1
    //
    // FUNCTION_CALL_OVERHEAD: 1 extra token per function/tool call for the call delimiter
    //
    // REPLY_PRIMING: 3 tokens added once at the end (<|start|>assistant<|message|>)
